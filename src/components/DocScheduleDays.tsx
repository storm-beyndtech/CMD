...
const DocScheduleDays: React.FC<ScheduleProps> = ({ daysWithInfo }) => {
	...
	return (
		<div className="w-full overflow-x-auto no-scrollbar p-[22px] rounded-[14px] bg-white">
			<h2 className="text-lg font-semibold text-[#383E49] mb-4">{dayjs().format("MMMM YYYY")}</h2>
							...
							<div className="flex justify-center items-center space-x-1 h-fit">
								...
								{!day.available && day.consultationCount === 0 ? (
									...
									<>
										...
										{day.concluded && <RiCheckDoubleFill className="text-secondary3 text-2xl" />}

										{/* Unavailable icon */}
										{!day.available && <PiWarningOctagon className="text-secondary3 text-2xl" />}

										{/* Partial unavailability icon */}
										{day.partUnavailable && <PiWarningOctagon className="text-secondary3 text-2xl" />}
									</>
								)}
							</div>
						</div>

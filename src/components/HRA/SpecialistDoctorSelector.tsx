...
import { Availability, PrimaryPracticeLocation, User } from "./HRAAppointment";
	...
	return (
		<div className="flex flex-col gap-4">
			...
			<div>
				...
				<p className="text-[#667085] my-1 text-sm">Select the type of specialist you are looking for</p>
				<select
					id="specialist"
					className="input"
			...
			<div className="flex items-center gap-2 relative">
				...
				>
					<LuCheck className={`text-[12px] ${manualDoctorSelection ? "block text-secondary3" : "hidden"}`} />
				</div>
				<label className="z-[1] font-medium text-[#383E49] pl-8">Select your specialist manually</label>
			</div>

			{/* Doctor Selector */}
			{manualDoctorSelection && (
				<div className="py-5">
					<p className="text-lg font-semibold text-[#383E49]">Available Specialists</p>
					<p className="text-[#667085] my-1 text-sm">Select a specialist you would like attending to you</p>
					{doctors.map((doctor) => (
						<div
							key={doctor._id}
						...
						>
							<input
								...
								onChange={() => onDoctorSelect(doctor._id)}
								className="hidden" // Hide the default radio button
							/>
							<div
								className={`w-5 h-5 border-[1.5px] rounded-full flex items-center justify-center ${
									selectedDoctor === doctor._id ? "border-secondary3" : "border-gray-300"
							...
							<Avatar
								firstName={doctor.user.firstName || "K"}

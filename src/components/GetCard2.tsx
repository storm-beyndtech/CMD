...
export default function GetCard2() {
	...
	return (
		<div>
			{isAvailable ? (
				<div className="flex flex-wrap gap-8 items-center bg-white p-6 rounded-[14px]">
					...
					<div className="grid pt-1">
						...
						<p className="text-[#5D6679] mb-10">
							<span className="text-secondary3 font-semibold">${cardInfo.balance}</span> CareCredits available
						</p>
				...
				<div className="bg-white p-6 rounded-[14px] flex flex-wrap-reverse justify-between items-center gap-2">
					...
					<div className="w-full max-w-[490px] flex flex-col gap-2">
						<h2 className="text-2xl font-semibold text-[#383E49]">Get your Carepoints Card</h2>
						<p className="text-[#5D6679] leading-[25.5px] w-full mb-1">
							{" "}
							Our card is packed with exclusive benefits designed for those who expect nothing but the best in
						...
						<div className="flex gap-3">
							...
							<Link to="#" className="flex items-center gap-3 hover:gap-4 text-secondary font-semibold">
			...
			{isModalOpen && (
				<CardDetails isModalOpen={isModalOpen} setIsModalOpen={setIsModalOpen} cardDetails={cardInfo} />
			)}
		</div>
	);
}

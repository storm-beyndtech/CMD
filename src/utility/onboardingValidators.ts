export const docSpecialtyValidatorV1 = (values: {
  ...
  if (medicalLicenseNumber.length < 3)
    return "License number should be at least 3 digits";
  ...
  if (isNaN(experienceYearsNumber))
    return "Experience years must be a valid number";
  if (experienceYearsNumber < 1 || experienceYearsNumber > 50)
...
export const docAvailabilityValidator = (values: {
  ...
  if (!availability.allDay) {
    if (!availability.startTime)
      return "Start time is required if not available all day";
    if (!availability.endTime)
      return "End time is required if not available all day";

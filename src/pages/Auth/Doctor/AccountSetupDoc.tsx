...
export default function AccountSetupDoc() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		const validAvailability = formValues.availability.filter(
			(schedule: any) =>
				schedule.allDay || (schedule.startTime.length === 5 && schedule.endTime.length === 5),
		);
		...
		try {
			...
			await sendRequest("/doctors/profile/complete-account-setup", "POST", {
				...formValues,
	...
	return (
		<div className="w-full min-h-screen bg-primary pb-10 px-5">
			...
			<div className="w-full flex flex-col items-center">
				<h2 className="title sm:!text-[22px] !text-xl text-center my-7">Complete Account Setup</h2>

...
import { validateLoginFields, validateRegisterFields } from "../../../utility/validators";
...
export default function PatientAuth() {
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			setLoading(true);
			const res = await sendRequest(`/auth/${activeTab}`, "POST", reqData);
			...
			if (activeTab === "register") {
				...
				if (res.data.user.accountType === "partner") {
	...
	return (
		<div className="flex min-h-screen bg-primary">
			...
			<div className="hidden md:flex justify-start w-[40%]">
				<img src={family} alt="auth" className="w-auto h-screen fixed top-0 left-0" />
			</div>

			{/* Right: Form Content */}
			<FormLayout activeTab={activeTab} setActiveTab={setActiveTab} page="patient" error={error}>
				<AuthForm
